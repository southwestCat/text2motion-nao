...
length = 53
sample = torch.from_numpy(np.load("A_person_raises_his_right_arm_and_then_lowers_it.npy")[:53])[None, :, :]
joints = feats2joints(sample)[0].detach().cpu().numpy()
joints = motion_temporal_filter(joints, sigma=2.5)
print(joints.shape)
np.save("001.npy", joints)