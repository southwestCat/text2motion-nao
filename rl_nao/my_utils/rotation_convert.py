...
def homogeneous_bmm(quat_a:torch.Tensor, pos_a:torch.Tensor, quat_b:torch.Tensor, pos_b:torch.Tensor) -> torch.Tensor:
    """
    Homogenerous matrix multiply.
    wThip = wTbase * baseThip
        quat_a: wRbase
        pos_a: wpbase
        quat_b: baseRhip
        pos_b: basephip
    ...
    Returns:
        quat, pos
    """
    rotm_a = quaternion_to_matrix(quat_a)
    rotm_b = quaternion_to_matrix(quat_b)
    rotm = torch.bmm(rotm_a,rotm_b)
    quat = matrix_to_quaternion(rotm)
    pos = pos_a+torch.bmm(rotm_a,pos_b.unsqueeze(dim=-1)).squeeze(dim=-1)
    return quat,pos
    
def homogeneous_inverse(quat: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    quat0 = quaternion_invert(quat)
    pos0 = -quaternion_apply(quat0, pos)
    return quat0,pos0


def homogeneous_multiply(quat_a:torch.Tensor, pos_a:torch.Tensor, quat_b:torch.Tensor, pos_b:torch.Tensor) -> torch.Tensor:
    quat = quaternion_multiply(quat_a,quat_b)
    pos = pos_a + quaternion_apply(quat_a,pos_b)
    return quat,pos
